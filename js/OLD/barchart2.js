class Barchart {
     ...
     * @param {Array}
     */
    constructor(_config, _data, _x, _y) {
    ...
    updateVis() {
      ...
      const aggregatedDataMap = d3.rollups(vis.data, v => v.length, d => d[vis.x]);
      vis.aggregatedData = Array.from(aggregatedDataMap, ([key, count]) => ({ key, count }));

      //add extra info for filtering
      vis.aggregatedData.forEach(function(e){
        e["title"] = vis.x;
        e ["k"] = String(e.key) + vis.x;
      });
      ...
      bars
          .on('mouseover', (event,d) => {
            d3.select('#tooltip')
              ...
              .html(`<div class="tooltip-label">${vis.y}</div>${d3.format(',')(d.key)}`);
          })
          .on('mousemove', (event) => {
            d3.select('#tooltip')
